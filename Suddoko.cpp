...
/* A utility function to check whether a given Sudoku is valid or not. */
bool isValidSudoku(int grid[N][N]) {
    // Check each cell for a valid value
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            // If the current cell is filled
            if (grid[i][j] != 0) {
                // Check if the current value is valid in its row, column, and box
...
int main()
{
    ...
    if (isValidSudoku(grid)) {
        ...
        std::cout << "The provided Sudoku puzzle is invalid." << std::endl;
    }
    return 0;
}
